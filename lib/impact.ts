...
export function calculateImpact(facility: Facility, distanceMiles: number): ImpactResult {
  ...
  const lower = Math.round(components.reduce((total, component) => total + component.lower, 0))
  const upper = Math.min(100, Math.max(lower, Math.round(components.reduce((total, component) => total + component.upper, 0))))
  return { lower, upper, midpoint: Math.round((lower + upper) / 2), label: upper >= 80 ? 'Very high' : upper >= 60 ? 'High' : upper >= 40 ? 'Moderate' : 'Low', components, regionalEffects: ['Electricity rates, regional water availability, and utility expansion are tracked separately and are not distance-decayed parcel impacts.'] }
}
