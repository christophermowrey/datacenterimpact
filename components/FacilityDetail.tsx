...
const scoreComponents: Record<string, [string, string]> = {
  scale: ['Facility scale and site context', 'Known facility scale, footprint, and site context'],
  electricity: ['Electricity and grid', 'Reported load, substations, and interconnection evidence'],
  generation: ['Generation and air', 'Generation type, permits, and documented capacity'],
  water: ['Water and cooling', 'Cooling method and reported water information'],
  noise: ['Noise and vibration', 'Cooling, generators, transformers, and complaints'],
  vibration: ['Vibration', 'Generators, transformers, construction, and engineering evidence'],
  landUse: ['Land and wildlife', 'Site footprint, flood context, and land sensitivity'],
}
...
export default function FacilityDetail({ facility }: { facility: Facility }) {
  ...
  const impactClass = impactTone(immediateImpact.upper, immediateImpact.upper)
  ...
  return <main className="detail-page">
    ...
    <div className="detail-wrap">
      ...
      <div className="community-summary">
        ...
        <aside className={`score-panel community-impact-panel ${impactClass}`}><p className="eyebrow">COMMUNITY IMPACT</p><div className="big-score">{immediateImpact.lower}–{immediateImpact.upper}<small> / 100</small></div><strong>{immediateImpact.label} potential impact at approximately 1/4 mile</strong><div className="evidence-summary">Evidence quality: <b>{evidenceLabel}</b></div><button className="outline-button full" onClick={() => setShowWhy((value) => !value)}>How this score works <span>{showWhy ? '↑' : '↓'}</span></button>{showWhy && <div className="why-panel"><p>Each row shows the distance-adjusted contribution to the displayed range. The upper bound includes conservative proxies when important facts are incomplete. Percentages show how much each category can contribute to the 100-point score.</p>{immediateImpact.components.map((component) => { const [name, reason] = scoreComponents[component.category] ?? [component.category, component.rationale]; return <div className="component-row" key={component.category}><span><b>{name}</b><small>{reason}</small></span><span className="component-score"><b>{Math.round(component.lower)}–{Math.round(component.upper)} / 100</b><small>{component.weight}% of total score</small></span></div> })}<p>Water, electricity, air, sound, vibration, land/wildlife, and scale/site context are weighted separately. Regional water and electricity floors remain significant at distance.</p></div>}</aside>
      </div>
