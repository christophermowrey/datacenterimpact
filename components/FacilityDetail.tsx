...
export default function FacilityDetail({ facility }: { facility: Facility }) {
  ...
  const immediateImpact = calculateImpact(facility, 0.25)
  ...
  return <main className="detail-page"><header className="topbar"><Link href="/" className="brand"><span className="brand-mark">D</span><span>DATA CENTER <i>IMPACT</i></span></Link><div className="detail-actions"><button className="text-button" onClick={share}>Copy/share page ↗</button><Link href="/" className="back">← Back to map</Link></div></header><div className="detail-wrap">
    ...
    <div className="detail-grid"><section><MapView facilities={[facility]} selected={selected} onSelect={(next) => setSelected((current) => current.slug === next.slug ? current : next)} searchedLocation={null} /><h2>What we know</h2><p className="detail-intro">{facility.summary}</p><div className="facts">{facility.metrics?.map((metric) => <div key={metric.label}><small>{metric.label.toUpperCase()}</small><strong>{metric.value}</strong>{metric.note && <span>{metric.note}</span>}</div>)}{power && <div className={power.significant ? 'power-fact significant' : 'power-fact'}><small>ELECTRICITY SCALE</small><strong>{power.label}</strong><span>Based on listed power capacity, not disclosed actual consumption. The project threshold for significant electricity demand is 100 homes.</span></div>}<div><small>OPERATOR / DEVELOPER</small><strong>{facility.operator}</strong></div><div><small>LOCATION PRECISION</small><strong>{facility.locationPrecision === 'exact' ? 'Verified address' : 'Approximate location'} {facility.locationPrecision !== 'exact' && <span className="tooltip-wrap"><button className="tooltip-button" aria-label="Why is this location approximate?">i</button><span className="tooltip-popup" role="tooltip">The reviewed official source does not publish a precise street address. The map point represents the general campus area.</span></span>}</strong></div><div><small>ADDRESS / AREA</small><strong>{facility.address ?? `${facility.city}, ${facility.county} County`}</strong></div></div></section><aside className="score-panel"><p className="eyebrow">IMMEDIATE NEIGHBORHOOD SCENARIO</p><div className="big-score">{immediateImpact.lower}–{immediateImpact.upper}</div><strong>{immediateImpact.label} potential impact at approximately 1/4 mile</strong><p>This conservative range is distance-sensitive and not a prediction of property value, health outcomes, or legal risk. A searched address recalculates the range from its actual distance.</p><div className="confidence-row"><span>Evidence confidence</span><b>{facility.confidence}</b></div><button className="outline-button full" onClick={() => setShowWhy((value) => !value)}>Why this range? <span>{showWhy ? '↑' : '↓'}</span></button>{showWhy && <div className="why-panel"><p><b>How to read it</b></p><p>The lower end reflects supported inputs. The upper end includes conservative proxies for important missing facts.</p>{scoreComponents.map(([name, weight, reason]) => <div className="component-row" key={name}><span><b>{name}</b><small>{reason}</small></span><b>{weight}</b></div>)}<p>Local components decay with distance. A small regional electricity baseline remains because data-center demand enters the interconnected Houston/Texas power system.</p></div>}</aside></div>
    ...
    <p className="disclaimer">Data Center Impact is informational and may contain incomplete or changing information. Always conduct independent legal, environmental, appraisal, and inspection due diligence.</p>
  </div></main>
}
