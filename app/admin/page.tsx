  ...
  return (
    <main className="admin-page">
      ...
      <section className="admin-panel" aria-labelledby="candidate-heading">
        ...
        {candidateQueue.error ? <p className="admin-note">{candidateQueue.error}</p> : candidateQueue.rows.length === 0 ? <p className="admin-note">No candidate records are currently imported.</p> : <div style={{ overflowX: 'auto' }}><table className="admin-table"><thead><tr><th>Name</th><th>Lifecycle</th><th>Confidence</th><th>Location</th><th>Sources</th><th>Reports</th><th>Reviewed</th></tr></thead><tbody>{candidateQueue.rows.map((candidate) => <tr key={candidate.entityId}><td><strong>{candidate.name}</strong><small>{candidate.slug}</small></td><td>{candidate.lifecycleStatus}</td><td>{candidate.confidence}</td><td>{candidate.locationPrecision}{candidate.city ? ` · ${candidate.city}` : ''}</td><td>{candidate.sourceCount}</td><td>{candidate.reportCount}</td><td>{candidate.lastReviewedAt ? new Date(candidate.lastReviewedAt).toLocaleDateString() : 'Never'}</td></tr>)}</tbody></table></div>}
